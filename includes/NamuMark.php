		...
		switch (strtolower($text)) {
			...
			default:
				...
				if (self::startsWithi($text, 'age') && preg_match('/^age\((.+)\)$/i', $text, $include)) {
					$include[1] = str_replace(',', '|', $include[1]);
					$include[1] = urldecode($include[1]);
					$birth = strtotime($include[1]);
					return floor((date('Ymd') - date('Ymd', $birth)) / 10000);
				}
		...
		for ($i = $offset; $i < $len; $i = self::seekEndOfLine($text, $i) + 1) {
			...
			for ($j = 1; $j < $td_cnt - 1; $j++) {
				...
				while (self::startsWith($innerstr, '<') && !preg_match('/^<[^<]*?>([^<]*?)<\/.*?>/', $innerstr) && !self::startsWithi($innerstr, '<br')) {
					$dummy = 0;
					$prop = $this->bracketParser($innerstr, $dummy, array('open' => '<', 'close' => '>', 'multiline' => false, 'processor' => function ($str) {
						return $str;
					}));
	...
	protected function textProcessor($otext, $type)
	{
		...
		switch ($type) {
			...
			case '{{{':
				...
				} elseif (preg_match('/^\+([1-6]) (.*)$/', $text, $size)) {
					for ($i = 1; $i <= $size[1]; $i++) {
