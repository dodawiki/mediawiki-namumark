...
class DDarkMark {
  ...
  public static function NamuMarkHTML( Parser &$parser, &$text ) {
  ...
  public static function NamuMarkHTML2( &$parser, &$text ) {
  	$title = $parser->getTitle();
  	if (!preg_match('/^특수:/', $title) && !preg_match("/&action=history/", $_SERVER["REQUEST_URI"]) && !preg_match('/^사용자:.*\.(css|js)$/', $title)) {
  ...
  public static function NamuMarkExtraHTML ( &$parser, &$text ) {
  	$title = $parser->getTitle(); // 문서의 제목을 title로 변수화한다.
